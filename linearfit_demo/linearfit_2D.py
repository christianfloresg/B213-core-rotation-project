...
def main():
    ...
    mome_one_file = filename.replace(filename.split('/')[-1], 'velocity_gradient_' +source_name + '_v_range_' + str(vrange[0])+'_'+str(vrange[1]) + '.mom1.fits')
    ...
    outfig=True, outname = source_name + '_gradient_direction' + '_v_range_' + str(vrange[0])+'_'+str(vrange[1])
               )
